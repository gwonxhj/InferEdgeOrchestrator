...
- worker는 engine deserialization과 execution context creation 성공 후
  tensor metadata inspection까지 마친 뒤 input/output buffer binding 및 inference
  execution에 대한 의도적인 not-implemented boundary에서 멈춘다.
...
1. identity model input/output buffer binding
