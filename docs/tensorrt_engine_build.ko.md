...
- `reports/jetson_tensorrt_runtime_telemetry.json`이 생성된다.
- worker smoke result는 `PASS_TENSORRT_INFERENCE`이다.
- runtime telemetry result는 `PASS_TENSORRT_TELEMETRY`이다.
- worker는 identity-model frame 1개를 실행하고 `output_preview`를 포함한 TensorRT
  backend metadata를 반환한다.
- runtime telemetry의 `result_events[].output`은 TensorRT backend metadata를
  포함한다.
...
2. multi-task TensorRT contention 상황에서 scheduler/load-shedding behavior 검증
3. TensorRT telemetry shape가 review에 충분히 안정되면 작은 sample telemetry
   artifact로 정리
...
Diversified TensorRT contention milestone은 외부 download가 아니라 생성된 ONNX
source model 2개를 사용한다. Build-only helper는 다음이다.
...
| --- | --- | --- |
| Detector-like ONNX | `models/generated/detector_tiny.onnx` | commit하지 않는다. |
| Classifier-like ONNX | `models/generated/classifier_tiny.onnx` | commit하지 않는다. |
| Detector-like TensorRT engine | `models/generated/detector_tiny_fp16.plan` | commit하지 않는다. |
| Classifier-like TensorRT engine | `models/generated/classifier_tiny_fp16.plan` | commit하지 않는다. |
| Build note | `reports/jetson_tensorrt_diverse_engine_build.md` | raw report는 commit하지 않는다. |
