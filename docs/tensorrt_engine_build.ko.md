...
- worker는 여전히 engine deserialization과 inference execution에 대한 의도적인
  not-implemented boundary에서 멈춘다.
...
1. `TensorRtWorker` 뒤에 TensorRT engine deserialization 구현
2. execution context 생성
3. identity model input/output buffer binding
4. scheduler contract를 바꾸지 않고 worker result metadata 반환
5. 같은 smoke path를 다시 실행해 실제 Jetson execution evidence 기록
