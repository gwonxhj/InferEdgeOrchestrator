...
transport로 표현할 수 있다. 기본값에서는 starter가 network connection을 열지
않고, `--execute-plan`을 명시했을 때만 단일 HTTP POST 또는 SSH command를
실행해 structured execution result를 기록할 수 있다.
