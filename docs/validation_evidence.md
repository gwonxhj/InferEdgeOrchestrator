...
| Jetson device-local sustained starter | Vision image, Voice request, and Safety process resource producers with ONNX Runtime probe, live `tegrastats`, runtime event summary, and standalone EdgeEnv feed contract validation on Jetson Orin Nano | PASS, starter | [`docs/jetson_smoke_test.md`](jetson_smoke_test.md#device-local-sustained-starter-smoke) |
