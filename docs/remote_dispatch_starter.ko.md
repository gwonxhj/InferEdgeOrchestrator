...
- file-based worker registry와 task request ingestion
- decision reason을 포함한 selected/rejected worker evidence
- 기본 `plan_only` remote execution context
- `--execute-plan`을 명시했을 때만 수행되는 제한된 HTTP/SSH starter 실행
- task retry policy가 허용할 때만 기록되는 bounded fallback recovery evidence
...
- cloud control plane 또는 Kubernetes-style scheduling

리뷰 의미:

- Orchestrator는 operation evidence를 생산한다.
- AIGuard는 이 evidence가 downstream으로 전달될 때 deterministic warning
  context를 설명할 수 있다.
- 최종 deployment decision owner는 Lab이다.
