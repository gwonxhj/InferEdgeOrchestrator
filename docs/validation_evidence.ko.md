...
| Jetson TensorRT guard smoke | Jetson Orin Nano에서 local identity ONNX를 TensorRT engine으로 생성하고 TensorRT worker guard boundary 검증 | PASS | [`docs/tensorrt_engine_build.ko.md`](tensorrt_engine_build.ko.md) |
...
- TensorRT Python binding availability와 TensorRT worker engine-file guard
  behavior를 검증한다.
- 실제 engine deserialization과 inference execution을 추가하기 전 현재 worker가
  기대한 not-implemented boundary에 도달하는지 확인한다.
...
이 결과는 TensorRT setup과 worker guard path를 검증한다. TensorRT inference
execution, GPU provider validation, TensorRT contention 상황의 scheduler behavior,
performance benchmark는 아직 검증하지 않는다.
...
- Jetson TensorRT guard smoke는 engine creation과 worker guard-boundary behavior만
  증명한다. TensorRT inference execution은 아직 증명하지 않는다.
