...
| Jetson device-local sustained starter | Jetson Orin Nano에서 Vision image, Voice request, Safety process resource producer를 ONNX Runtime probe, live `tegrastats`, runtime event summary, standalone EdgeEnv feed contract와 함께 검증 | PASS, starter | [`docs/jetson_smoke_test.ko.md`](jetson_smoke_test.ko.md#device-local-sustained-starter-smoke) |
