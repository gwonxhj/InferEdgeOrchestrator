...
상태: schema, TensorRT engine deserialization, Jetson guard-smoke validation
문서다. config schema, TensorRT worker deserialization path,
...
TensorRT inference execution, GPU provider execution, multi-task TensorRT
...
engine을 deserialize하고 cache할 수 있지만, TensorRT inference execution은 아직
구현되지 않았다.
...
현재 TensorRT prerequisite을 확인하고 설정된 engine을 deserialize한 뒤, inference
...
- `worker="tensorrt"`는 TensorRT가 설정된 engine을 deserialize하지 못하면
  명확한 오류로 실패해야 한다.
- `worker="tensorrt"`는 deserialized engine을 engine path 기준으로 cache한다.
- `scripts/smoke_jetson_tensorrt.sh`는 Jetson에서 dependency inventory capture,
  config validation, TensorRT engine deserialization, inference execution에 대한
  현재 기대 not-implemented boundary 도달 여부를 확인할 수 있다.
...
- worker는 engine deserialization 성공 후 inference execution에 대한 현재의 명확한
...
deserialization path가 다음 구현 단계로 넘어갈 준비가 되었는지만 증명한다.
