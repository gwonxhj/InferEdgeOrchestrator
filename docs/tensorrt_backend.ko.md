...
config schema는 이제 향후 `tensorrt` worker 선택을 허용하며, 기존 `dummy`,
`onnxruntime` config와의 backward compatibility를 유지한다. 단, 이것은 schema
contract일 뿐이고 TensorRT worker 실행은 아직 구현되지 않았다.
...
이 예시는 config schema로는 유효하다. 단, `worker="tensorrt"` runtime execution은
TensorRT worker가 추가되기 전까지 명확한 not-implemented 오류로 실패한다.
...
실제 worker 추가 시 더해야 할 validation rule:
