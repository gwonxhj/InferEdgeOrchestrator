...
tensor metadata inspection, Jetson guard-smoke validation 문서다. config schema,
TensorRT worker deserialization/context/metadata path,
`scripts/smoke_jetson_tensorrt.sh`가 존재하며, Jetson guard smoke는 local
`models/identity_fp16.plan` engine으로 `PASS_GUARD_STUB`에 도달했다. 단,
TensorRT input/output buffer binding, inference execution, GPU provider
...
tensor metadata를 기록하고 runtime object를 engine path 기준으로 cache할 수 있지만,
TensorRT input/output buffer binding과 inference execution은 아직 구현되지 않았다.
...
context를 생성하고 input/output tensor metadata를 기록한 뒤, input/output buffer
binding과 inference execution이 추가되기 전까지 명확한 not-implemented 오류로
실패한다.
...
- `scripts/smoke_jetson_tensorrt.sh`는 Jetson에서 dependency inventory capture,
  ...
  tensor metadata inspection, input/output buffer binding 및 inference execution에
  대한 현재 기대 not-implemented boundary 도달 여부를 확인할 수 있다.
...
- worker는 engine deserialization, execution context creation, tensor metadata
  inspection 성공 후 input/output buffer binding과 inference execution에 대한 현재의
  명확한 not-implemented boundary에 도달해야 한다.
- script는 ignored `reports/` 아래에 local report를 작성한다.

이 결과는 TensorRT inference evidence가 아니다. Jetson 환경, engine artifact,
deserialization path, execution context path, tensor metadata path가 다음 구현
단계로 넘어갈 준비가 되었는지만 증명한다.
