...
| EdgeEnv Telemetry Feed Candidate | Additive sustained report block that maps Orchestrator queue/deadline/fallback/resource context, including `max_total_queue_depth`, into an EdgeEnv runtime telemetry context candidate | `edgeenv_runtime_telemetry_feed` in sustained reports |
