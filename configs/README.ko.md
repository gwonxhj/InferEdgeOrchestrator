...
| [`jetson_tensorrt_smoke.json`](jetson_tensorrt_smoke.json) | Jetson에서 예약된 TensorRT worker guard path를 확인한다. | TensorRT/GPU backend guard | `ENGINE_PATH=models/detector.plan scripts/smoke_jetson_tensorrt.sh` | `reports/jetson_tensorrt_guard_validation.md` 및 `reports/` 아래 dependency inventory. | TensorRT Python binding과 device-local engine file이 필요하다. guard smoke 초안이며 TensorRT inference evidence는 아니다. |
...
- TensorRT/GPU backend inference 실행은 아직 구현되지 않았다. config schema는
  해당 경로를 위해 `worker="tensorrt"`, `engine_path`, `worker_options`를
  예약했고, worker layer에는 import/file guard stub이 추가되었으며,
  [`scripts/smoke_jetson_tensorrt.sh`](../scripts/smoke_jetson_tensorrt.sh)는
  Jetson dependency와 guard-boundary evidence를 ignored `reports/` 아래에
  기록한다. 자세한 내용은
