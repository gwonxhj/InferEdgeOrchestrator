...
- TensorRT/GPU backend 실행은 아직 구현되지 않았지만, config schema는 해당
  경로를 위해 `worker="tensorrt"`, `engine_path`, `worker_options`를 예약했다.
  자세한 내용은 [`docs/tensorrt_backend.ko.md`](../docs/tensorrt_backend.ko.md)에
  기록한다.
