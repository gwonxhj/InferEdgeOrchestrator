...
| [`agent_multi_workload_sustained_voice_ingress.json`](agent_multi_workload_sustained_voice_ingress.json) | Vision file producer와 FastAPI-style local request fixture를 함께 실행해 Voice workload producer로 전달한다. | Reliable Edge Agent Runtime: Voice Ingress Producer Starter | `python3 -m inferedge_orchestrator run-multi-workload-sustained --config configs/agent_multi_workload_sustained_voice_ingress.json --output reports/agent_multi_workload_sustained_voice_ingress.json --frames 16` | Voice `producer_source=fastapi_request_fixture`, selected routes, request digest, burst workload evidence를 담은 `reports/agent_multi_workload_sustained_voice_ingress.json`. | 작은 JSON request fixture를 사용하며, 실제 FastAPI service 또는 Whisper backend 실행 전 ingress producer 단계다. |
| [`agent_multi_workload_sustained_safety_resource.json`](agent_multi_workload_sustained_safety_resource.json) | Vision file, Voice ingress, Safety resource snapshot producer를 함께 실행한다. | Reliable Edge Agent Runtime: Safety Monitor Producer Starter | `python3 -m inferedge_orchestrator run-multi-workload-sustained --config configs/agent_multi_workload_sustained_safety_resource.json --output reports/agent_multi_workload_sustained_safety_resource.json --frames 16` | Safety `producer_source=resource_snapshot_fixture`, CPU/memory/temperature signal, fallback/deadline signal, degradation score를 담은 `reports/agent_multi_workload_sustained_safety_resource.json`. | live monitor/tegrastats producer를 붙이기 전 작은 resource snapshot fixture를 사용한다. |
| [`agent_multi_workload_sustained_device_local.json`](agent_multi_workload_sustained_device_local.json) | Vision image, Voice request, Safety resource producer를 명시적 `device_local` mode로 실행한다. | Reliable Edge Agent Runtime: Device-Local Sustained Starter | `python3 -m inferedge_orchestrator run-multi-workload-sustained --config configs/agent_multi_workload_sustained_device_local.json --output reports/agent_multi_workload_sustained_device_local.json --frames 16` | `producer_sources`, `device_local_producer_count`, producer-backed sustained evidence를 담은 `reports/agent_multi_workload_sustained_device_local.json`. | live YOLO/ONNX, FastAPI, tegrastats, Jetson/RPi producer 전 committed local fixture를 사용하는 starter다. |
...
- `agent_multi_workload_sustained_voice_ingress.json`은 첫 Voice ingress producer
  단계다. 작은 request fixture를 읽고 FastAPI server 없이 route, request, burst
  evidence를 기록한다.
- `agent_multi_workload_sustained_safety_resource.json`은 첫 Safety monitor
  producer 단계다. 작은 resource snapshot fixture를 읽고 CPU, memory, temperature,
  queue, fallback, deadline signal을 기록한다.
- `agent_multi_workload_sustained_device_local.json`은 명시적 `device_local`
  starter다. 기본 실행은 dependency-free로 유지하면서 committed Vision image,
